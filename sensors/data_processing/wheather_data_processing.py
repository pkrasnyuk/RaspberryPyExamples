...
class WheatherDataProcessing(BaseDataProcessing):
    ...
    def processing(self) -> None:
        ...
        try:
            while True:
                wheather_data: Optional[DtoWeather] = self.__dht_sensor_service.get_sensor_data()
                if wheather_data is not None:
                    first_line_message: Optional[str] = (
                        f"Temp:      {wheather_data.temperature:0.1f}C"
                        if wheather_data.temperature is not None
                        else None
                    )
